...
async def on_ready() -> None:
     ...
     await channel.send("Client login successful") 
...
async def on_connect() -> None:
    ser = len(bot.guilds)
    await bot.change_presence(activity=discord.Streaming(name=f"{ser} servers!!!"
, url = "https://kalai.systems"))
...
async def on_command_completion(context: Context) -> None:
    ...
    if context.guild is not None:
        print(f'Executed {executed_command} command in **{context.guild.name} ({context.channel.name})** by ** {context.author} **')
    else:
